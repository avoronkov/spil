...
func TestLazyList(t *testing.T) {
	var ll List = NewLazyList(EvalerFunc(testCounter, TypeList), Int64(0), false)
	res := make([]Expr, 0, 10)
	for !ll.Empty() {
		val, err := ll.Head()
		if err != nil {
...
func TestLazyListState(t *testing.T) {
	var ll List = NewLazyList(EvalerFunc(fibGen, TypeList), QList(Int64(1), Int64(1)), false)
	res := make([]Expr, 0, 10)
	for i := 0; i < 6; i++ {
		val, err := ll.Head()
		if err != nil {
...
func TestLazyListFiniteString(t *testing.T) {
	ll := NewLazyList(EvalerFunc(testCounter, TypeAny), Int64(0), false)
